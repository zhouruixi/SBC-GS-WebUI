...
$(document).ready(function () {
    ...
    function loadDroneWfbConfig() {
        ...
        $.get("/load_drone_config/wfb", function (data) {
            ...
            resultDiv.html(`<div class="alert alert-success alert-dismissible fade show" role="alert" id="load-result-drone-wfb-success-alert">
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                加载配置成功！
                                </div>`);
            // 设置 3 秒后自动消失
            setTimeout(function () {
                $('#load-result-drone-wfb-success-alert').alert('close');
            }, 2000);
            // resultDiv.html(`<div class="alert alert-success">读取配置成功！</div>`);
            // alert("加载配置成功！");
        }).fail(function () {
            alert("加载 Drone wfb 配置失败，请手动重新加载！");
        });
        // }
    }
    ...
    function saveDroneWfbConfig() {
        ...
        $.ajax({
            ...
            success: function (response) {
                const resultDiv = $("#save-result-drone-wfb");
                if (response.success) {
                    resultDiv.html(`<div class="alert alert-success alert-dismissible fade show" role="alert" id="save-result-drone-wfb-success-alert">
                                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                        ${response.message}
                                        </div>`);
                    // 设置 3 秒后自动消失
                    setTimeout(function () {
                        $('#save-result-drone-wfb-success-alert').alert('close');
                    }, 2000);
                } else {
                    resultDiv.html(`<div class="alert alert-danger">${response.message}</div>`);
                }
    ...
    function loadDroneMajesticConfig() {
        ...
        $.get("/load_drone_config/majestic", function (data) {
            ...
            resultDiv.html(`<div class="alert alert-success alert-dismissible fade show" role="alert" id="load-result-drone-majestic-success-alert">
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                加载配置成功！
                                </div>`);
            // 设置 3 秒后自动消失
            setTimeout(function () {
                $('#load-result-drone-majestic-success-alert').alert('close');
            }, 2000);
            // resultDiv.html(`<div class="alert alert-success">读取配置成功！</div>`);
            // alert("加载配置成功！");
        }).fail(function () {
            alert("加载 Drone Majestic 配置失败，请手动重新加载！！");
        });
        // }
    }
    ...
    function saveDroneMajesticConfig() {
        ...
        $.ajax({
            ...
            success: function (response) {
                const resultDiv = $("#save-result-drone-majestic");
                if (response.success) {
                    resultDiv.html(`<div class="alert alert-success alert-dismissible fade show" role="alert" id="save-result-drone-majestic-success-alert">
                                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                        ${response.message}
                                        </div>`);
                    // 设置 3 秒后自动消失
                    setTimeout(function () {
                        $('#save-result-drone-majestic-success-alert').alert('close');
                    }, 2000);
                } else {
                    resultDiv.html(`<div class="alert alert-danger">${response.message}</div>`);
                }
